const TOPICS = {
  ...
  restassured: {
    ...
    bank: [
      ...
      {q:'Як налаштувати Jackson для серіалізації?',o:['RestAssured.config().objectMappingConfig(ObjectMappingConfig.objectMappingConfig().jackson2ObjectMapperFactory(...))','RestAssured.jackson = new ObjectMapper()','given().mapper(new ObjectMapper())','RestAssured.objectMapper = new ObjectMapper()'],c:0,e:'objectMappingConfig з jackson2ObjectMapperFactory дозволяє кастомізувати ObjectMapper глобально.'},
      ...
      {q:'Як встановити кодування для запиту?',o:['RestAssured.config().encoderConfig(EncoderConfig.encoderConfig().defaultContentCharset("UTF-8"))','given().charset("UTF-8")','RestAssured.charset = "UTF-8"','given().encoding("UTF-8")'],c:0,e:'EncoderConfig.encoderConfig().defaultContentCharset("UTF-8") задає кодування для тіла запиту.'},
  ...
  qaautomation: {
    ...
    bank: [
      ...
      {q:'Що таке bug report?',o:['Структурований опис дефекту: кроки відтворення, очікуваний/фактичний результат, середовище','Лог помилки','Автоматичний звіт тестів','Email розробнику'],c:0,e:'Хороший bug report: Title, Steps to Reproduce, Expected Result, Actual Result, Environment, Severity, Priority, Attachments.'},
  ...
  javacore: {
    ...
    bank: [
      ...
      {q:'Яка різниця між abstract class та interface?',o:['Abstract class може мати стан/конструктор, клас може успадкувати тільки один; interface — множинне успадкування','Немає різниці з Java 8','Abstract class не може мати реалізації методів','Interface може мати конструктор'],c:0,e:'Abstract class: стан, конструктор, одиночне успадкування. Interface: множинна реалізація, з Java 8 — default/static методи.'},
      ...
      // ── Колекції ──
      {q:'Яка різниця між ArrayList та LinkedList?',o:['ArrayList — масив (швидкий get, повільний insert/delete в середині); LinkedList — двозв\'язний список (повільний get, швидкий insert/delete)','LinkedList швидший у всьому','ArrayList займає менше пам\'яті завжди','Немає різниці'],c:0,e:'ArrayList: O(1) get, O(n) insert/delete в середині. LinkedList: O(n) get, O(1) insert/delete при наявності вузла.'},
      ...
      {q:'Яка різниця між Comparable та Comparator?',o:['Comparable — клас сам визначає "природний" порядок (compareTo); Comparator — зовнішній об\'єкт для кастомного сортування','Вони ідентичні','Comparator вбудований в клас','Comparable для чисел, Comparator для рядків'],c:0,e:'Comparable: клас реалізує compareTo() — один "природний" порядок. Comparator: окремий об\'єкт, кілька різних порядків.'},
      ...
      {q:'Яка різниця між Queue та Deque?',o:['Queue — FIFO з одного кінця; Deque — Double Ended Queue, додавати/видаляти з обох кінців','Queue швидша','Deque тільки LIFO','Немає різниці'],c:0,e:'Queue: offer/poll/peek (FIFO). Deque: offerFirst/offerLast/pollFirst/pollLast — може бути і черга і стек.'},
      // ── Stream API ──
      ...
      {q:'Яка різниця між intermediate та terminal операціями Stream?',o:['Intermediate — ледачі, повертають Stream; terminal — запускають обробку, повертають результат','Intermediate швидші','Terminal повертають Stream','Немає різниці'],c:0,e:'Intermediate (filter, map, sorted) — lazy, не виконуються без термінальної. Terminal (collect, forEach, count) — запускають pipeline.'},
      ...
      {q:'Яка різниця між checked та unchecked виключеннями?',o:['Checked — компілятор вимагає обробки (try-catch або throws); unchecked (RuntimeException) — не вимагає','Checked — серйозніші','Unchecked потрібно оголошувати в throws','Немає різниці'],c:0,e:'Checked: IOException, SQLException — треба try-catch або throws у сигнатурі. Unchecked (RuntimeException): NullPointerException, IndexOutOfBoundsException.'},
      ...
      {q:'Яка різниця між Runnable та Callable?',o:['Runnable.run() — void без виключень; Callable.call() — повертає результат, може кидати checked виключення','Runnable підтримує return','Callable не підтримує виключення','Немає різниці'],c:0,e:'Runnable: void run(). Callable<V>: V call() throws Exception — повертає Future<V> при submit в ExecutorService.'},
  ...
  playwright: {
    ...
    bank: [
      ...
      {q:'Як підключити Page Object через fixture?',o:['test.extend<{ loginPage: LoginPage }>({ loginPage: async ({ page }, use) => { await use(new LoginPage(page)) } })','import LoginPage у тесті','@Inject LoginPage','extend(LoginPage)'],c:0,e:'test.extend() дозволяє передавати Page Object як fixture — автоматично ін\'єктується у тест без ручного new.'},
      ...
      {q:'Що таке optional chaining (?.) у TypeScript/JS?',o:['user?.address?.city — повертає undefined замість помилки якщо проміжне значення null/undefined','Умовний виклик','Аналог оператора ?:','Тільки для методів'],c:0,e:'obj?.prop?.method?.() — безпечна навігація по потенційно null/undefined ланцюжку. Замінює if (obj && obj.prop && ...).'},
  ...
  sysdesign: {
    ...
    bank: [
      ...
      {q:'Яка різниця між монолітом та мікросервісами?',o:['Моноліт — один процес, легший у розробці; мікросервіси — розподілені, складніші але масштабованіші','Моноліт завжди повільніший','Мікросервіси завжди кращі','Немає різниці'],c:0,e:'Моноліт: просто розробляти, важко масштабувати частково. Мікросервіси: незалежний деплой, але складніші тестування і debugging.'},
      ...
      {q:'Що таке CDN (Content Delivery Network)?',o:['Мережа розподілених серверів що доставляє статичний контент з найближчого до користувача вузла','Черга для доставки повідомлень','Розподілена база даних','Мережа мікросервісів'],c:0,e:'CDN кешує статику (JS, CSS, images) на edge серверах по всьому світу. Зменшує latency та навантаження на origin сервер.'},
      ...
      {q:'Що таке REST vs gRPC?',o:['REST — HTTP/JSON, зрозумілий людині; gRPC — бінарний протокол (Protobuf), швидший, типізований','REST завжди швидший','gRPC тільки для internal','Немає різниці'],c:0,e:'REST: text-based, легко debug, широка підтримка. gRPC: ефективніший (binary), streaming, code generation, типізація через Protobuf.'},
      ...
      {q:'Що таке CAP теорема?',o:['Розподілена система може гарантувати лише 2 з 3: Consistency, Availability, Partition tolerance','Три обовязкові властивості БД','Принципи REST API','Правила мікросервісів'],c:0,e:'CAP: під час network partition треба вибирати між C (всі вузли бачать однакові дані) та A (система відповідає).'},
      ...
      {q:'Що таке індекс у базі даних?',o:['Структура даних що прискорює пошук за рахунок додаткового місця та повільніших writes','Обмеження на дані','Зовнішній ключ','Схема таблиці'],c:0,e:'Індекс (B-tree, Hash): SELECT швидший, але INSERT/UPDATE/DELETE — повільніші. Надмірні індекси шкодять write performance.'},
      ...
      {q:'Що таке кешування і для чого воно?',o:['Зберігання часто запитуваних даних у швидкому сховищі (RAM) для зменшення навантаження на БД','Збереження логів','Черга запитів','Копія бази даних'],c:0,e:'Cache (Redis, Memcached): latency мкс замість мс. Cache-aside, Write-through, Write-behind — стратегії.'},
      ...
      {q:'Яка різниця між RabbitMQ та Kafka?',o:['RabbitMQ — традиційна черга (push, видаляє після споживання); Kafka — лог подій (pull, зберігає, replay)','Kafka швидший у всьому','RabbitMQ масштабованіший','Немає різниці'],c:0,e:'RabbitMQ: message broker, routing, Dead Letter Queue. Kafka: event streaming, high throughput, retention, consumer groups, replay.'},
      ...
      {q:'Що таке service mesh?',o:['Інфраструктурний шар для управління сервіс-до-сервіс комунікацією (mTLS, tracing, retries)','Мережа мікросервісів','API Gateway','Load balancer'],c:0,e:'Service mesh (Istio, Linkerd): sidecar proxy для кожного сервісу. Обробляє: mTLS, circuit breaking, retries, observability без змін коду.'},
      ...
      {q:'Що тестувати у мікросервісах специфічно?',o:['Contract testing, service isolation, network failures, eventual consistency, idempotency','Тільки unit тести','Тільки E2E тести','Тільки performance'],c:0,e:'Мікросервіси: contracts між сервісами, поведінка при недоступності залежностей, retry логіка, idempotency, eventual consistency state.'},
      ...
      {q:'Що таке Saga патерн?',o:['Розподілені транзакції через послідовність локальних транзакцій з компенсаційними діями при помилці','Стан-машина','Message queue патерн','Retry патерн'],c:0,e:'Saga: Order → Reserve Inventory → Charge Payment → Ship. При помилці — compensating transactions. Choreography або Orchestration.'},
  ...
  aiqa: {
    ...
    bank: [
      ...
      {q:'Які обмеження AI при генерації тестів?',o:['Не розуміє бізнес-логіку, може генерувати неправильні assertions, hallucinate API, не знає домен','AI завжди правий','AI не може генерувати тести','AI повільніший за ручне написання'],c:0,e:'AI обмеження: незнання бізнес-правил, hallucination методів, поверхові тести без edge cases, потреба в review та валідації QA.'},
      ...
      {q:'Що таке AI-assisted code review для тестів?',o:['Аналіз тестового коду AI для знаходження проблем: відсутні assertions, погані назви, missing edge cases','Автоматичний merge','Заміна code review','Тільки стиль коду'],c:0,e:'AI code review: знаходить відсутні assertions, погану ізоляцію тестів, відсутні edge cases, naming issues. Доповнює але не замінює людський review.'},
      ...
      {q:'Що таке data drift у ML?',o:['Зміна розподілу вхідних даних у production порівняно з тренуванням — знижує точність моделі','Витік тренувальних даних','Оновлення моделі','Зміна схеми БД'],c:0,e:'Data drift: модель навчена на даних 2022, production 2024 — розподіл змінився → accuracy падає. Моніторинг: PSI, KS test.'},
      ...
      {q:'Що таке prompt injection?',o:['Вставка шкідливих інструкцій у prompt щоб змусити LLM ігнорувати системні обмеження','SQL injection аналог','XSS атака','CSRF атака'],c:0,e:'Prompt injection: "Ignore previous instructions and..." — тестувати на ігнорування system prompt, витік системних інструкцій, небажані дії.'},
      ...
      {q:'Що таке test oracle problem у AI тестуванні?',o:['Складність визначення "правильної" відповіді для AI системи — немає однозначного expected result','Проблема тестових даних','Проблема локаторів','Проблема середовища'],c:0,e:'Test oracle problem: для "опиши це зображення" правильних відповідей багато. Потрібні метрики (BLEU, ROUGE) або human evaluation.'},
      {q:'Що таке metamorphic testing для AI?',o:['Перевірка відносин між вхідними даними: якщо трохи змінити input — output повинен змінитись передбачувано','Тестування метаданих','Трансформаційні тести','API тестування'],c:0,e:'Metamorphic: "якщо rotate зображення кішки на 5°, класифікація повинна залишатись кішкою". Вирішує test oracle problem.'},
      {q:'Що таке explainability тестування?',o:['Перевірка що AI пояснює свої рішення зрозуміло та правильно (SHAP, LIME, attention maps)','Документація моделі','Тестування логіки','Аудит коду'],c:0,e:'XAI testing: чи пояснення відповідають реальній логіці моделі? SHAP values, LIME — перевіряємо чи feature importance правдоподібна.'},
      {q:'Яка роль QA при впровадженні AI у продукт?',o:['Тестування AI функцій, визначення метрик якості, bias testing, A/B тести, моніторинг у production','Тільки функціональне тестування','Тільки unit тести моделі','AI тестує себе'],c:0,e:'QA для AI: define quality metrics (accuracy, fairness), edge cases dataset, regression при оновленні моделі, production monitoring.'},
      ...
      {q:'Що таке continuous testing з AI?',o:['AI аналізує зміни коду та автоматично запускає релевантні тести та генерує нові при потребі','Постійний запуск всіх тестів','CI/CD pipeline','Моніторинг production'],c:0,e:'AI continuous testing: зміна коду → AI виявляє affected areas → запускає targeted тести → при нових функціях пропонує нові тест-кейси.'},
      ...
      {q:'Що таке responsible AI testing?',o:['Перевірка AI систем на відповідність етичним принципам: fairness, transparency, privacy, safety','Тестування безпеки тільки','Compliance тестування','Тестування ліцензій'],c:0,e:'Responsible AI testing: fairness metrics, bias detection, privacy (PII exposure), safety (harmful output), transparency (explainability).'},
      ...
      {q:'Як AI змінює роль QA інженера?',o:['Від ручного написання тестів до визначення стратегії, review AI output, тестування AI систем','Повністю замінює QA','Не змінює','Робить QA непотрібним'],c:0,e:'QA еволюція: менше boilerplate написання → більше стратегія, exploratory testing, AI output review, тестування AI продуктів, quality advocacy.'},
    ]
  },
  ...
  qabasics: {
    ...
    bank: [
      ...
      {q:'Що таке Test Case?',o:['Документований набір вхідних даних, передумов, кроків, очікуваних результатів для однієї перевірки','Скрипт тесту','Вимога','Bug report'],c:0,e:'Test Case: Test ID, Title, Preconditions, Steps, Expected Result, Actual Result, Status, Priority. Мінімальна одиниця тестування.'},
      ...
      {q:'Яка різниця між Severity та Priority?',o:['Severity — технічний вплив (blocker/critical/major/minor); Priority — бізнес-важливість (P1/P2/P3)','Вони однакові','Severity важливіший','Priority технічний'],c:0,e:'Critical severity + Low priority: crash у рідко використовуваній функції. Low severity + High priority: опечатка у назві компанії на головній.'},
      ...
      {q:'Що таке three amigos у Agile?',o:['BA + Developer + QA обговорюють user story разом перед розробкою для спільного розуміння','Три тестувальники','Три рівні тестів','Три критерії якості'],c:0,e:'Three amigos: BA пояснює що потрібно, DEV пропонує як реалізувати, QA виявляє edge cases та ризики. Запобігає непорозумінням.'},
      ...
      {q:'Що таке Acceptance Criteria (AC)?',o:['Конкретні умови які User Story повинна задовольняти для прийнятності — основа тест-кейсів','Критерії завершення тестування','Технічні вимоги','NFR вимоги'],c:0,e:'AC формат Given/When/Then або чекліст: "Given user is logged in, When clicks logout, Then redirected to login page and session cleared".'},
      {q:'Що таке BDD (Behavior-Driven Development)?',o:['Підхід де специфікація у форматі Given/When/Then служить і документацією і автоматизованими тестами','Тестування тільки поведінки','Branch-Driven Development','Business Data Design'],c:0,e:'BDD (Cucumber, SpecFlow): Given/When/Then сценарії розуміють всі stakeholders + автоматично виконуються. Living documentation.'},
      {q:'Що таке TDD (Test-Driven Development)?',o:['Red → Green → Refactor: спочатку пишемо тест що падає, потім мінімальний код, потім рефакторинг','Test After Development','Тестування в окремій команді','Тестування всіх вимог'],c:0,e:'TDD: тест → fail (Red) → мінімальний код → pass (Green) → refactor. Покращує дизайн коду, 100% coverage за замовчуванням.'},
      ...
      {q:'Що таке smoke test vs sanity test?',o:['Smoke — поверхова перевірка основних функцій після деплою; Sanity — глибока перевірка конкретної фічі після fix','Вони ідентичні','Smoke глибший','Sanity — після деплою'],c:0,e:'Smoke: "чи запустилось?" — login, home page, main navigation. Sanity: "чи цей fix працює?" — вузька перевірка конкретного виправлення.'},
      ...
      {q:'Що таке localization та internationalization тестування?',o:['i18n — готовність до локалізації (дати, валюти, кодування); l10n — переклади, культурні особливості','Вони ідентичні','Тільки переклад','Тільки дати'],c:0,e:'i18n (internationalization): UTF-8, date formats, RTL підтримка. l10n (localization): правильні переклади, валюти, телефони.'},
      {q:'Що таке Recovery тестування?',o:['Перевірка відновлення системи після збоїв: відключення живлення, мережеві помилки, DB crash','Тестування backup','Тестування rollback','Disaster recovery план'],c:0,e:'Recovery testing: як система реагує на crash? Чи відновлює стан? Чи показує правильні повідомлення? RTO/RPO відповідність.'},
      ...
      {q:'Що таке tester mindset (мислення тестувальника)?',o:['Критичне мислення, сумніви, пошук проблем, захист якості на відміну від developer mindset','Оптимізм','Написання тестів','Управління проектом'],c:0,e:'Tester mindset: "як це може зламатись?" Curiosity, skepticism, systematism, attention to detail, advocate for quality.'},
      ...
      {q:'Що таке flaky test та як з ним боротись?',o:['Нестабільний тест — то проходить то падає. Рішення: ізоляція, детермінізм, видалення race conditions','Тест що завжди падає','Тест без assertions','Тест з mock обєктами'],c:0,e:'Flaky tests: race conditions, time dependencies, shared state, network unreliability. Quarantine → analyse → fix → reintegrate.'},
